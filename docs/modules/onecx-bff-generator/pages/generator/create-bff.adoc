...
The flag `--autobuild` will build the application after generation. If you want to build it manually, you can omit this flag and run `mvn clean package` in the generated directory.

The flag `--name` is used to specify the name of the generated BFF, which will be used as base for project name, artifact name and some essential application parts.

The flag `--group` is used to specify the Maven `groupId` of the generated BFF project.

The flag `--package` is used to specify the base Java package name of the generated BFF project.

The flag `--frontend-api` is used to specify the path or URL to the frontend OpenAPI specification. This API represents the contract exposed by the generated BFF.

The flag `--backend-api` is used to specify the path or URL to the backend OpenAPI specification. This API represents the backend service consumed by the generated BFF.

The flag `--artifact-id` can be used to specify a custom Maven `artifactId`. If omitted, the artifact ID is derived from the project name.

The flag `--output-dir` can be used to specify a custom output directory for the generated project.

The flag `--parent-version` can be used to specify a concrete `onecx-quarkus3-parent` version. If omitted, the generator resolves the latest available release.
...
java -jar target/onecx-bff-generator-1.0.0-SNAPSHOT.jar create-bff \
  --name onecx-demo-bff \
  --group org.tkit.onecx \
  --package org.tkit.onecx.demo.bff \
  --frontend-api ~/projects/onecx/onecx-demo-ui/demo/src/assets/api/openapi-bff.yaml \
  --backend-api ~/projects/onecx/onecx-demo-svc/src/main/openapi/onecx-demo-svc-internal.yaml \
  --output-dir ~/projects/onecx \
  --autobuild
----
